...
* Local Model Adapter v1 with explicit worker-local loopback backends, bounded file inputs/responses, symbolic model ids, and no implicit fallback or cloud path: implementation complete on the active capability branch.
* Worker discovery, executable agents, and evaluation corpus remain open.
