...
For E2E work, agents should first confirm the repository has reached the 13/13 core standard. Default browser checks target only a locally started or reserved test environment and progress from Level 1 rendering, through Level 2 deterministic public UI, to Level 3 mocked or isolated business flows. Level 4 staging/provider-sandbox work needs separate explicit authority. Agents must not substitute a production URL when local startup fails, request production credentials, or exercise real providers merely to make an E2E check pass.
