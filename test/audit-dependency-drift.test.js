...
/** @param {string} name @param {string} pythonVersion */
function createPythonService(name, pythonVersion) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `${name}-`));
  fs.writeFileSync(path.join(root, ".python-version"), `${pythonVersion}\n`);
...
test("ecosystem formatter exposes the high-value version matrix", () => {
  ...
  assert.match(output, /REPOSITORY\s+NODE\s+PYTHON\s+PACKAGE_MANAGER/);
  for (const label of ["REACT", "NEXT", "VITE", "TYPESCRIPT", "SUPABASE", "PLAYWRIGHT"]) assert.match(output, new RegExp(label));
...
test("Python runtime drift is reported without inventing Node reproducibility gaps", () => {
  const first = createPythonService("python-drift-a", "3.12.8");
  const second = createPythonService("python-drift-b", "3.13.1");
  const report = inspectDependencyDrift([first, second]);
  const pythonDrift = report.runtimeDrift.find((item) => item.dimension === "python-version");
  assert.ok(pythonDrift);
  assert.equal(pythonDrift.variants.length, 2);
  assert.equal(report.summary.missingPackageManager, 0);
  assert.equal(report.summary.missingEngines, 0);
  assert.equal(report.summary.missingNodeVersion, 0);
  assert.equal(report.overallStatus, "WARN");
});

test("expanded framework and tooling catalog detects Next and Vite drift", () => {
  const first = createWebapp("framework-catalog-a", {
    packageManager: "bun@1.3.14", nodeVersion: "24.11.1", engines: { node: ">=24 <25" },
    dependencies: { next: "15.5.0" }, devDependencies: { vite: "7.1.0" },
  });
  const second = createWebapp("framework-catalog-b", {
    packageManager: "bun@1.3.14", nodeVersion: "24.11.1", engines: { node: ">=24 <25" },
    dependencies: { next: "16.0.0" }, devDependencies: { vite: "8.0.0" },
  });
  const report = inspectDependencyDrift([first, second]);
  assert.equal(report.drift.some((item) => item.dependency === "next"), true);
  assert.equal(report.drift.some((item) => item.dependency === "vite"), true);
  assert.equal(report.majorVersionConflicts.some((item) => item.dependency === "next"), true);
  assert.equal(report.majorVersionConflicts.some((item) => item.dependency === "vite"), true);
});
