...
`bun run agent:telemetry` validates and aggregates explicit agent-run telemetry for latency, CPU/GPU time, token usage, proposal outcomes, review effort, reopened defects, and optional evaluation-corpus outcomes. It groups observations by role, symbolic model, and worker without inventing a composite model-quality score. See `docs/agent-resource-telemetry.md`.
...
Embedded `authenticated` booleans are treated as caller metadata only. Release Evidence Bundle trust fields default to `false` and are upgraded only by `Signed Evidence Verification v1` over the exact evidence bytes. Signed mode uses an explicit public-key policy plus one or more detached signature envelopes; invalid, stale, wrong-kind, wrong-key, or tampered evidence produces a failing bundle check. The public toolkit never generates or stores signing private keys. See `docs/signed-evidence-verification.md` for the envelope, policy, freshness, and release-bundle trust boundaries.
